...
方法三：
    ...
    3.检测是否有初始化函数：spwan主动调用目标方法？错，应该是dlopen加载so后，执行JNI_Onload后再[主动调用]，
        * 如果报错或者返回结果不对，说明有初始化函数；
        * 否则，则大概率无初始化函数(有例外:如京东还是有初始化函数)
    4.定位初始化函数
        dlopen加载so后，执行JNI_Onload后hook目标类所有方法，对所有目标类Java函数hook，然后在每个函数执行前(后)都先[主动调用目标函数]，进行排查。

    注：如果目标类将其下某个静态字段设置为初始化状态，则可以用如下方式判断：
            1.结果正确的情况下，判断目标类所有静态字段的值
            2.结果错误的情况下，判断目标类所有静态字段的值
            3.使用Unidbg执行时，观察日志
